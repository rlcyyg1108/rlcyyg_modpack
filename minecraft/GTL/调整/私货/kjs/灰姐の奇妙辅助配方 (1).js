ServerEvents.recipes((event) => {
    ...
    const p2p_1 = ["redstone","item","fluid","fe"]
    const p2p_2 = ["pattern","eu"]
    const p2p_3 = ["me","light"]
    for (let i = 0; i < p2p_1.length; i++){     //p2p组装机配方
    gtr.assembler(`greythings:${p2p_1[i]}_p2p_tunnel`)
        .notConsumable(`ae2:${p2p_1[i]}_p2p_tunnel`)
        .itemInputs("3x minecraft:iron_ingot",
                    "ae2:engineering_processor",
                    "3x ae2:fluix_crystal"
        )
        .itemOutputs(`ae2:${p2p_1[i]}_p2p_tunnel`)
        .EUt(GTValues.VA[GTValues.MV])
        .duration(200)
    gtr.assembler(`greythings:${p2p_1[i]}_multi_p2p_tunnel`)
        .notConsumable(`mae2:${p2p_1[i]}_multi_p2p_tunnel`)
        .itemInputs("3x minecraft:iron_ingot",
                    "2x ae2:engineering_processor",
                    "3x ae2:fluix_crystal"
        )
        .itemOutputs(`mae2:${p2p_1[i]}_multi_p2p_tunnel`)
        .EUt(GTValues.VA[GTValues.MV])
        .duration(200)
    }
    for (let i = 0; i < p2p_2.length; i++){
    gtr.assembler(`greythings:${p2p_2[i]}_p2p_tunnel`)
        .notConsumable(`mae2:${p2p_2[i]}_p2p_tunnel`)
        .itemInputs("3x minecraft:iron_ingot",
                    "ae2:engineering_processor",
                    "3x ae2:fluix_crystal"
        )
        .itemOutputs(`mae2:${p2p_2[i]}_p2p_tunnel`)
        .EUt(GTValues.VA[GTValues.MV])
        .duration(200)
    gtr.assembler(`greythings:${p2p_2[i]}_multi_p2p_tunnel`)
        .notConsumable(`mae2:${p2p_2[i]}_multi_p2p_tunnel`)
        .itemInputs("3x minecraft:iron_ingot",
                    "2x ae2:engineering_processor",
                    "3x ae2:fluix_crystal"
        )
        .itemOutputs(`mae2:${p2p_2[i]}_multi_p2p_tunnel`)
        .EUt(GTValues.VA[GTValues.MV])
        .duration(200)
    }
    for (let i = 0; i < p2p_3.length; i++){
    gtr.assembler(`greythings:${p2p_3[i]}_p2p_tunnel`)
        .notConsumable(`ae2:${p2p_3[i]}_p2p_tunnel`)
        .itemInputs("3x minecraft:iron_ingot",
                    "ae2:engineering_processor",
                    "3x ae2:fluix_crystal"
        )
        .itemOutputs(`ae2:${p2p_3[i]}_p2p_tunnel`)
        .EUt(GTValues.VA[GTValues.MV])
        .duration(200)
    }

    gtr.assembler("greythings:trident") //三叉戟
        .circuit(24)
        .itemInputs("16x minecraft:prismarine_shard",
                    "4x gtceu:ruthenium_rod",
                    "2x gtceu:double_osmium_plate",
                    "gtceu:exquisite_diamond_gem"
        )
        .inputFluids("gtceu:glue 1440")
        .itemOutputs("minecraft:trident")
        .EUt(GTValues.VA[GTValues.EV])
        .duration(300)

    gtr.lightning_processor("greythings:channeling_trident")//引雷三叉戟
        .itemInputs("minecraft:trident",
            "4x minecraft:end_crystal"
        )
        .itemOutputs(Item.of('minecraft:trident', '{Damage:230}').enchant('minecraft:channeling', 1))
        .EUt(GTValues.VA[GTValues.EV])
        .duration(800)
})
